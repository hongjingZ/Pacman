...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance == None:
        d = {self.getJailPosition():1.0}
        allPossible.update(d)
  ...
  def elapseTime(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    allPossible = util.Counter()
    ##print "ghost_pos",gameState.getGhostPosition
    ##print "beliefs:",self.getBeliefDistribution()
    for oldPos in self.legalPositions:
        newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))##generate new_pos from oldPos
        for newPos, prob in newPosDist.items():            ##sum up all the newPos prob from oldPos
...
class ParticleFilter(InferenceModule):
  ...
  def initializeUniformly(self, gameState):
    ...
    legal_moves = len(self.legalPositions)
    for i in self.legalPositions:
        ##self.particles[i] = float(self.numParticles)/legal_moves
        self.particles[i] = 1
  ...
  def observe(self, observation, gameState):
    ...
    "*** YOUR CODE HERE ***"
    """
    print "PACMAN ob:",gameState.getPacmanPosition()
    ...
    if noisyDistance != None:
        ...
        resample = util.Counter()
        if sum(weights.values()) != 0.0: ## resample from all the particle weights
            for i in range(self.numParticles):
                resample[util.sample(weights)] += 1
        else:
            legal_moves = len(self.legalPositions)
            for i in self.legalPositions:
                ##resample[i] = float(self.numParticles/legal_moves)
                resample[i] = 1
        ...
        ##ghost get captured.
        d = util.Counter()
        d[self.getJailPosition()] = self.numParticles
        self.particles = d
    ##self.particles.normalize()
    """
    weights = util.Counter()
    if noisyDistance != None:
        for pos in self.legalPositions:
          trueDis = util.manhattanDistance(pos,pacmanPosition)
          belief = self.getBeliefDistribution()
          weights[pos] = weights[pos] + emissionModel[trueDis]*belief[pos]
        if max(weights.values()) == 0:
          self.initializeUniformly(gameState)
        else:
          resample = []
          for _ in range(self.numParticles):
            resample.append(util.sample(weights))
          self.particles = resample
    else:
        for i in range(self.numParticles):
  ...
  def elapseTime(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    """
    print "PACMAN:",gameState.getPacmanPosition()
    transition = {}
    for pos_par in self.particles:
        newPosDist = self.setGhostPosition(gameState,pos_par)
        transition[pos_par] = self.getPositionDistribution(newPosDist)

    temp = util.Counter()
    for particle in self.particles:
        for i in range(self.particles[particle]):
            temp[util.sample(transition[particle])] += 1
    self.particles = temp
    """
    temp = []
    for pos_par in self.particles:
        newPosDist = self.setGhostPosition(gameState,pos_par)
        transition = self.getPositionDistribution(newPosDist)
        temp.append(util.sample(transition))
    self.particles = temp
  ...
  def getBeliefDistribution(self):
    ...
    """
    "*** YOUR CODE HERE ***"
    ##sum up the elements in each particle and normalize it.
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    self.particles = []
    pPos = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
    print "pPos:",pPos
    random.shuffle(pPos)
    for i in range(self.numParticles):
      for pos in pPos:
    ...
    for index, particle in enumerate(self.particles):
        ...
        for i in range(self.numGhosts):
            if noisyDistances[i] != None:
                ##using likelihood of the noisy observations
                likehood = likehood * emissionModels[i][util.manhattanDistance(particle[i],pacmanPosition)]
            else:
                particle = list(particle)
                particle[i] = self.getJailPosition(i)
                particle = tuple(particle)
        ##print "particle",particle
        allPossible[particle] = allPossible[particle] + likehood
  ...
  def getBeliefDistribution(self):
    dist = util.Counter()
    for part in self.particles: dist[part] += 1.0
    dist.normalize()
    return dist
