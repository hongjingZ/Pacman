...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    for ghostdistribution in livingGhostPositionDistributions:
        max = -1
        for item in ghostdistribution:
            if ghostdistribution[item] > max:
                max = ghostdistribution[item]
                index = item
        Possible_place.append(index)
    ##self.distancer.getDistance(pos1,pos2)
    min_dist = 999999
    for action in legal:
        successorPosition = Actions.getSuccessor(pacmanPosition,action)
        pos_len = []
        for pos in Possible_place:
            pos_len.append(self.distancer.getDistance(successorPosition,pos))
        if min(pos_len) < min_dist:
            min_dist = min(pos_len)
            my_action = action
    return my_action