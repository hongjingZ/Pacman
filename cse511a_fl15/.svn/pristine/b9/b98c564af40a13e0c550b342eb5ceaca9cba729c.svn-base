...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    costs={state:0}
    optCost=9999999
    while problem.isGoalState(state)==False or queue.isEmpty==False:
        if problem.isGoalState(state)==True:
            if costs[state]<optCost:
                optCost=costs[state]
                continue
        succs=problem.getSuccessors(state)
        for item in succs:
            if item[0] not in visited:
                some=[item,state]  #remember parent
                queue.push(some, item[2]+costs[state]+heuristic(item[0],problem))
        while 1:
            test=queue.pop()
            if test[0][0] not in visited:
                break
        state=test[0][0]
        testFather=test[1]
        currPath=paths[testFather]+[test[0][1]]
        paths.update({state:currPath})
        costs.update({state:test[0][2]+costs[testFather]})
        visited.append(state)
