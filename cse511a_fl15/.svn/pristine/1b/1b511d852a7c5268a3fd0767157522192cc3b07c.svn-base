...
class AlphaBetaAgent(MultiAgentSearchAgent):
  ...
  def alphabeta(self,state,agent,depth,action,alpha,beta):
    ...
    if self.is_finish(state,depth):
        score = (self.evaluationFunction(state,depth),action)
...
def contestEvaluationFunction(currentGameState,depth):
  ...
  if len(curfood) > 0:
      ...
      w7 = 400
      ##if pacman is in the left tunal and eat all the food, it must get out soon.
      evaluation = is_dead * (-100000)+w1*min_dist + w2*max_dist + w3*(1/(min_g_dist+0.1)) + w4*(len(curfood)) + w5*(len(capsuleplaces)) + currentGameState.getScore()/float(2.0)+\
                   w6*kk + w7*eat_ghost
      return evaluation
  else:

      return 1000000+depth*(-200)
...
class ContestAgent(MultiAgentSearchAgent):
  ...
  def alphabeta(self,state,agent,depth,action,alpha,beta,a1,a2,a3):
    ...
    if agent == self.agentCount:
        agent = self.index
    if self.is_finish(state,depth):
        score = (contestEvaluationFunction(state,depth),action)
    elif agent == self.index:
